...
export default {
  ...
  methods: {
    ...
    deleteDriver (item) {
      this.$store.dispatch('surveys/deleteSurvey', item)
      .then((res) => {
        this.close() // close dialog
        if (res.response) {
          this.$emit('drawer') // close drawer
          this.$store.commit('mainDrivers/DELETE_MAIN_WITH_HEADERS', res.response) // update item
          this.$emit('removeItem', res.response)
        }
