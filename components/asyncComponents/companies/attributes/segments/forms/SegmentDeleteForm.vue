    ...
    <v-card>
      ...
      <v-card-text>
        ...
        <ul class="text-subtitle-2 my-3">
          <li>{{ childItem.name }} segment</li>
          <li>{{ childItem.type_id == 1 ? 'Option Type' : (childItem.type_id == 2 ? 'Employee Type' : 'Number Type') }}</li>
        </ul>
...
export default {
  ...
  methods: {
    ...
    async deleteSegment (item) {
      const req = await this.$store.dispatch('segments/deleteSegment', item)
      this.close()
    }
  }
}
